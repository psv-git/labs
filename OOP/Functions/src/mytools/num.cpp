...
int Mts::Add(int operand1, int operand2, bool& error) {
  ...
  int result = 0;
  if (operand2 > 0) { 
    if (operand1 > INT_MAX - operand2) error = true;
  } else if (operand2 < 0) {
    if (operand1 < INT_MIN - operand2) error = true;
  }
  if (!error) result = operand1 + operand2;
  return result;
}
  ...
  if (sign > 0) {
    if (operand1 > 0) {
      ...
      if (operand1 == -1 || operand2 == -1) {
        if (operand1 < -INT_MAX || operand2 < -INT_MAX) error = true;
      } else if (operand1 < INT_MAX / operand2) error = true;
    }
  } else if (sign < 0) {
    if (operand1 > 0) {
      if (operand2 != -1) {
...
int Mts::Div(int operand1, int operand2, bool& error) {
  ...
  if (operand2 == 0) error = true;
    ...
    if (operand1 < -INT_MAX) error = true;
  }
  if (!error) result = operand1 / operand2;
  return result;
}
...
double Mts::Add(double operand1, double operand2, bool& error) {
  ...
  if (result == DINF) error = true;
  else if (result == -DINF) error = true;
  return result;
}
...
std::wstring Mts::IntToStr(int number, int radix, bool &error) {
  error = false;
  std::wstring wstr;
  if (radix < 2 || radix > 36) error = true;
  else {
    ...
    do {
      quot = number / radix;
      rem = number % radix;
      if (negative) wstr.push_back(digit[Abs(rem)]);
      else wstr.push_back(digit[rem]);
      number = quot;
    } while (number);
    if (negative) wstr.push_back('-');
